...
if (blurrchecker(image) < 80.0):
    ...
    #cv2.waitKey(0)
    #cv2.destroyAllWindows()
    a = raw_input("do you need to rotate the image?")
    if a == "Yes":
        ang = int(input("Enter the rotation angle:"))
        image_new = rotation(warped_new,ang)
        cv2.imwrite('final_output.jpg',image_new)
    else:

        print("You didnt say anything else")
#the next part is to upload the image to the s3 server
    client = boto3.client('s3', region_name='ap-south-1')

    client.upload_file('var/www/html/python/final_output.jpg', 'dockboyz/uploads', 'final_output.jpg')
